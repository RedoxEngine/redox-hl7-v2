describe('Function: writeField', function() {
	...
	it('should handle simple fields like MSH.10', function(){
	  ...
	  it('should handle varies fields like OBX.5.2', function(){
		  field = {
			  2: 'PDF',
			  3: {
				  1: 'FAKE COMPONENT'
			  ...
			  5:{ 1: '|O|', 2 :'I AM A PDF DOCUME\\NT'
			  } 
		  };
		  ret = generator.writeField(field, 'OBX.5');
		  expect(ret).to.equal('^PDF^FAKE COMPONENT^&ANOTHER FAKE COMPONENT^\\F\\O\\F\\&I AM A PDF DOCUME\\E\\NT');
	  });
	  ...
	  it('should should handle null values for varies fields', function(){
		  ...
		  ret = generator.writeField(field, 'OBX.5');
		  expect(ret).to.equal('^PDF\\F\\^^&ANOTHER FAKE COMPONENT\\F\\^');
	  });
  
	  it('should escape HL7 control characters', function(){
		  ret = generator.writeField({1: '12345'}, 'PID.3');
		  expect(ret).to.equal('12345');
		  ...
		  ret = generator.writeField(field, 'PID.3');
		  expect(ret).to.equal('12345\\E\\^^^MR');
	  });
  });